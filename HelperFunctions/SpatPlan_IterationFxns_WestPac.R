...
loopthrough_EM_ClimatePriorityArea <- function(solution_list, metric_list, scenario_list) {
  ...
  for(scenario_num in 1:length(scenario_list)) {
    for(metric_num in 1:length(metric_list)) {
      ...
      ggSol <- fSpatPlan_PlotSolution(s_plot, PUs, land) + 
          ggtitle(paste0("Climate-smart design: ", metric_list[metric_num]), 
                  subtitle = "Climate Priority Area, SSP ", scenario_list[scenario_num]) + 
          theme(axis.text = element_text(size = 25))
...
loopthrough_MM_Penalty <- function(solution_list, metric_list, scenario_list, model_list) {
  ...
  for(scenario_num in 1:length(scenario_list)) {
    for(metric_num in 1:length(metric_list)) {
      for(model_num in 1:length(model_list)) {
        ...
        (ggSol <- fSpatPlan_PlotSolution(s_plot, PUs, land) + 
            ggtitle(paste0("Climate-smart design: ", metric_list[metric_num]), 
                    subtitle = paste0("Penalty, SSP", scenario_list[scenario_num]), " (GCM: ", model_list[model_num]) + 
            theme(axis.text = element_text(size = 25)))
...
loopthrough_MM_Feature <- function(solution_list, metric_list, scenario_list, model_list) {
...
loopthrough_MM_ClimatePriorityArea <- function(solution_list, metric_list, scenario_list, model_list) {
